...
class CanteenView extends ConsumerStatefulWidget {
...
class _CanteenViewState extends ConsumerState<CanteenView> {
  ...
  Future<List<Presences>> fetchPresence() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? url = prefs.getString("server");
    String key = prefs.getString(url!)!;
  ...
  late final Future<List<Presences>> myServices;
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<ChildData>>(
        ...
        builder: (context, snapshot) {
          if(snapshot.hasData){
            return FutureBuilder<List<Presences>>(
                future: myServices,
                      ...
                      builder: (context, snapshot2) {
                        if(snapshot2.hasData){
                          return StudentsList(
                            ...
                            presenceStudent: snapshot1.data == null ? []: snapshot1.data!,
                          );
                        }
                        else{
